...
var startNotes = ['C', 'C#', 'Db', 'D', 'D#', 'Eb', 'E', 'F', 'F#', 'Gb', 'G', 'G#', 'Ab', 'A#', 'Bb', 'B'];
...
function noteOn(note, velocity) {
  ...
  var velocityGain = ctx.createGain();
...
var naturalNotesSharps = ['C', 'D', 'F', 'G', 'A'];
var naturalNotesFlats = ['D', 'E', 'G', 'A', 'B'];
var range = ['C2', 'C6'];
var app = {
  setupPiano: function setupPiano() {
    var _this = this;
    var piano = document.querySelector("#piano");
    var allNaturalNotes = this.getNaturalNotes(range);
    var pianoWidth = allNaturalNotes.length * whiteKeyWidth;
    var SVG = this.createMainSVG(pianoWidth, pianoHeight);

    //add white keys
    var whiteKeyPositionX = 0;
    allNaturalNotes.forEach(function (noteName) {
      var whiteKeyTextGroup = utils.createSVGElement("g");
      var whiteKey = _this.createKey({
        className: "white-key",
        width: whiteKeyWidth,
        height: pianoHeight
      });
      var text = utils.createSVGElement("text");
    ...
    // add black keys
    var blackKeyPositionX = 60;
    allNaturalNotes.forEach(function (naturalNotes, index, array) {
      //  if last iteration of keys, do not add black key
      if (index === array.length - 1) {
        return;
      }
      var blackKeyTextGroup = utils.createSVGElement("g");
      var blackKey = _this.createKey({
        className: "black-key",
        width: whiteKeyWidth / 2,
        height: pianoHeight / 1.6
      });
      var flatNameText = utils.createSVGElement("text");
      var sharpNameText = utils.createSVGElement("text");
      utils.setAttributes(blackKeyTextGroup, {
        "width": whiteKeyWidth / 2
      });
      for (var i = 0; i < naturalNotesSharps.length; i++) {
        var naturalSharpNoteName = naturalNotesSharps[i];
        var naturalFlatNoteName = naturalNotesFlats[i];
        if (naturalSharpNoteName === naturalNotes[0]) {
          utils.setAttributes(blackKey, {
            "x": blackKeyPositionX,
            "data-sharp-name": "".concat(naturalSharpNoteName, "#").concat(naturalNotes[1]),
            "data-flat-name": "".concat(naturalFlatNoteName, "b").concat(naturalNotes[1]),
            "rx": 8,
            "ry": 8
          });
          ...
          utils.addTextContent(sharpNameText, "".concat(naturalSharpNoteName, "#"));
          utils.addTextContent(flatNameText, "".concat(naturalFlatNoteName, "b"));
  ...
  createOctave: function createOctave(octNum) {
    // create a group element with a class of octive, just like in html
    var octave = utils.createSVGElement("g");
    octave.classList.add("octave");

    // Moves the new octives that are created
    octave.setAttribute("transform", "translate(".concat(octNum * octWidth, ", 0)"));
    return octave;
  },
  createKey: function createKey(_ref) {
    var className = _ref.className,
      width = _ref.width,
      height = _ref.height;
    var key = utils.createSVGElement("rect");
  ...
  getNaturalNotes: function getNaturalNotes(_ref2) {
    var _ref3 = _slicedToArray(_ref2, 2),
      firstNote = _ref3[0],
      lastNote = _ref3[1];
    // Assingn octave number, notes and positoins to variables
    var firstNoteName = firstNote[0];
    var firstOctaveNumber = parseInt(firstNote[1]);
    var lastNoteName = lastNote[0];
    var lastOctaveNumber = parseInt(lastNote[1]);
    var firstNotePosition = naturalNotes.indexOf(firstNoteName);
    var lastNotePosition = naturalNotes.indexOf(lastNoteName);
    var allNaturalNotes = [];
    var _loop = function _loop(octaveNumber) {
      // first octave
      if (octaveNumber === firstOctaveNumber) {
        naturalNotes.slice(firstNotePosition).forEach(function (noteName) {
        ...
        naturalNotes.slice(0, lastNotePosition + 1).forEach(function (noteName) {
          allNaturalNotes.push(noteName + octaveNumber);
        });
      } else {
        naturalNotes.forEach(function (noteName) {
          allNaturalNotes.push(noteName + octaveNumber);
        });
      }
    };
    for (var octaveNumber = firstOctaveNumber; octaveNumber <= lastOctaveNumber; octaveNumber++) {
      _loop(octaveNumber);
    }
    return allNaturalNotes;
  },
  createMainSVG: function createMainSVG(pianoWidth, pianoHeight) {
    ...
    utils.setAttributes(svg, {
      ...
      "viewBox": "0 0 ".concat(pianoWidth, " ").concat(pianoHeight)
    });
    return svg;
  },
  displayNotes: function displayNotes(notes) {
    ...
    notes.forEach(function (noteName) {
      pianoKeys.forEach(function (key) {
        var naturalName = key.dataset.noteName;
        var sharpName = key.dataset.sharpName;
        var flatName = key.dataset.flatName;
...
var utils = {
  createSVGElement: function createSVGElement(el) {
    var element = document.createElementNS("http://www.w3.org/2000/svg", el);
    return element;
  },
  ...
  addTextContent: function addTextContent(el, content) {
    el.textContent = content;
  },
  removeClassFromNodeCollection: function removeClassFromNodeCollection(nodeCollection, classToRemove) {
